...
def _post_process_pdf_text(text: str) -> str:
    ...
    for line in merged_lines:
        # 檢查是否是新段落的開始
        if (line.startswith('#') or  # 標題
            line.endswith('：') or   # 中文冒號結尾
            line.endswith(':') or    # 英文冒號結尾
            len(line) > 150 or       # 長句通常是段落開始
            ...
            for v in part_vecs:
                try:
                    ...
                    if len(v_norm) != dimension:
                        print(f"[WARNING] 向量維度異常: 期望{dimension}, 實際{len(v_norm)}")
                        # 用零向量填充或截斷
                        if len(v_norm) < dimension:
                    ...
                    print(f"[WARNING] 向量轉換失敗: {e}, 使用零向量")
                ...
                try:
                    ...
                    for v in small_vecs:
                        ...
                        try:
                            ...
                            print(f"[WARNING] 小批次向量轉換失敗: {e2}")
                            vecs.append([0.0] * dimension)
                except Exception as e2:
                    print(f"[WARNING] 小批次 {j//8} 仍然失敗: {e2}")
                    # 為這個小批次建立零向量
                    for _ in range(len(small_part)):
                        vecs.append([0.0] * dimension)
            
            if not success:
                print(f"[ERROR] 批次 {i} 完全失敗，使用零向量")
...
def embed_query(query: str) -> List[float]:
    ...
    try:
        ...
        if len(result) != dimension:
            print(f"[WARNING] Query向量維度異常: 期望{dimension}, 實際{len(result)}")
            # 用零向量填充或截斷
            if len(result) < dimension:
